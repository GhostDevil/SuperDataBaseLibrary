...
namespace SuperDataBase
{
    ...
    /// 日 期:2015-06-25
    /// 作 者:不良帥
    /// 描 述:Odbc操作数据库辅助类（适用于 Oracle，Access，Excel，Visual FoxPro，SqlServer，MSSql，MSText，PostgreSQL，SQLLite，dBase，MySql）
    /// </summary>
    public sealed class DBOdbcHelper
    {
        ...
        public static void PrepareCommand(OdbcCommand cmd, OdbcConnection conn, OdbcTransaction trans, string cmdText, OdbcParameter[] cmdParms)
        {
        ...
        public static T ExecuteEntity<T>(string commandText, CommandType commandType, params OdbcParameter[] param)
        {
            T obj = default(T);
            using (OdbcConnection connection = new OdbcConnection(connectionString))
            {
                using (OdbcCommand cmd = new OdbcCommand(commandText, connection))
                {
        ...
        public static List<T> ExecuteList<T>(string commandText, CommandType commandType, params OdbcParameter[] param)
        {
        ...
        /// <returns>返回OdbcParameter对象</returns>
        public static OdbcParameter GetParm(string parmName, OdbcType parmType, object parmValue)
        {
            OdbcParameter parm = new OdbcParameter();
            parm.OdbcType = parmType;
        ...
        //数据库连接字符串
        //public static readonly string conn = "Provider=Microsoft.Jet.Odbc.4.0;Data Source=" + HttpContext.Current.Request.PhysicalApplicationPath + System.Configuration.ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
        ...
        /// <remarks>
        ///举例:  
        ///  int result = ExecuteNonQuery(connString, "PublishOrders", new OdbcParameter("@prodid", 24));
        /// </remarks>
        /// <param name="conn">一个现有的数据库连接</param>
        /// <param name="commandText">存储过程名称或者sql命令语句</param>
        /// <param name="commandParameters">执行命令所用参数的集合</param>
        /// <returns>执行命令所影响的行数</returns>
        public static int ExecuteNonQuery(OdbcConnection connection, string cmdText, params OdbcParameter[] commandParameters)
        {
            OdbcCommand cmd = new OdbcCommand();
        ...
        /// <remarks>
        ///举例:  
        ///  int result = ExecuteNonQuery(trans, "PublishOrders", new OdbcParameter("@prodid", 24));
        /// </remarks>
        /// <param name="trans">一个现有的事务</param>
        /// <param name="cmdText">存储过程名称或者sql命令语句</param>
        /// <param name="commandParameters">执行命令所用参数的集合</param>
        /// <returns>执行命令所影响的行数</returns>
        public static int ExecuteNonQuery(OdbcTransaction trans, string cmdText, params OdbcParameter[] commandParameters)
        {
            OdbcCommand cmd = new OdbcCommand();
        ...
        /// <remarks>
        /// 举例:  
        ///  OdbcDataReader r = ExecuteReader(connString, "PublishOrders", new OdbcParameter("@prodid", 24));
        /// </remarks>
        /// <param name="connectionString">一个有效的连接字符串</param>
        /// <param name="cmdText">存储过程名称或者sql命令语句</param>
        /// <param name="commandParameters">执行命令所用参数的集合</param>
        ...
        public static DataSet ExecuteDataSet(string connectionString, string cmdText, params OdbcParameter[] commandParameters)
        {
        ...
        /// <remarks>
        ///例如:  
        ///  Object obj = ExecuteScalar(connString, "PublishOrders", new OdbcParameter("@prodid", 24));
        /// </remarks>
        ///<param name="connectionString">一个有效的连接字符串</param>
        /// <param name="commandText">存储过程名称或者sql命令语句</param>
        /// <param name="commandParameters">执行命令所用参数的集合</param>
        ...
        /// <param name="commandParameters">执行命令所用参数的集合</param>
        /// <returns>用 Convert.To{Type}把类型转换为想要的 </returns>
        public static object ExecuteScalar(OdbcConnection connection, string cmdText, params OdbcParameter[] commandParameters)
        {
            OdbcCommand cmd = new OdbcCommand();
            PrepareCommand(cmd, connection, null, cmdText, commandParameters);
            object val = cmd.ExecuteScalar();
        ...
        public static OdbcParameter[] GetCachedParameters(string cacheKey)
        {
            ...
            for (int i = 0, j = cachedParms.Length; i < j; i++)
                clonedParms = (OdbcParameter[])((ICloneable)cachedParms).Clone();
            return clonedParms;
        }
        ///// <summary>
        ///// 准备执行一个命令
        ...
        ///// <param name="cmdText">命令文本,例如：Select * from Products</param>
        ///// <param name="cmdParms">执行命令的参数</param>
        //private static void PrepareCommand(OdbcParameter cmd, OleDbConnection conn, OleDbTransaction trans, string cmdText, OdbcParameter[] cmdParms)
        //{
        //    //判断连接的状态。如果是关闭状态，则打开
        //    if (conn.State != ConnectionState.Open)
        //        conn.Open();
